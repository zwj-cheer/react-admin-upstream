...
- React 19、React Router 7、TypeScript 6、Vite 8、pnpm 10
- TanStack Query 5、Zustand 5、Zod、react-hook-form
...
│   │   ├── ui/                  #   基础组件（button、select、dialog、form、toast…）
...
│   ├── app/                     # 组装层｜router、routeRegistry（合并模板与项目路由）、
│   │                            #   RequireAuth / RequirePermission 守卫；唯一允许注入
│   │                            #   src/project/ 值的地方
│   ├── project/                 # 项目拥有｜下游只改这里：
│   │   ├── branding.ts          #   品牌缩写标记与 edition（显示名称来自 runtime.json 的 app.name）
│   │   ├── navigation.ts        #   导航元数据
│   │   ├── routes.tsx           #   projectRoutes 数组，注册新页面（组件+元数据+能力）
...
├── scripts/                     # 模板 / 容器 / 扩展边界校验脚本
└── docs/                        # 项目文档（全部中文）
```

核心约束：模板目录不能导入 `src/project/`（由 `scripts/check-extension-boundaries.mjs`
强制），项目值只能经 `src/app/` 注入，保证下游升级不产生冲突。
